...
wxString ReservoirLevels::cellValue(int x, int y) const
{
    if (!pArea)
        return wxString();
    auto& matrix = pArea->hydro.series->reservoirLevels.standardReservoirLevelMatrix;
...
void ReservoirLevels::internalAreaChanged(Antares::Data::Area* area)
{
    ...
    if (pHydro)
    {
        MatrixAncestorType::matrix(&pHydro->series->reservoirLevels.standardReservoirLevelMatrix);
    }
    else
    {
